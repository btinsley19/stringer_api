...
export function sanitizePost(post: any): Post | null {
  ...
  return {
    ...
    user_involved: post.user_involved || false,
    event: post.event ? post.event.trim() : null,
    bias_score: post.bias_score || '0.0',
    embeddings_id: post.embeddings_id || 'unknown',
    created_at: post.created_at,
    updated_at: post.updated_at,
    location_is_address: post.location_is_address || false,
    external_source: post.external_source || false,
    source_url: post.source_url ? post.source_url.trim() : null,
    source_id: post.source_id || 'unknown',
    external_url: post.external_url ? post.external_url.trim() : null,
    location_geom: post.location_geom || null,
    formatted_address: post.formatted_address || null,
    bias_score_sentences: post.bias_score_sentences || [],
    bias_score_sentence_scores: post.bias_score_sentence_scores || [],
    organization: post.organization || null,
    category: post.category ? post.category.trim() : null,
    processed: post.processed || false,
    bias_labels: post.bias_labels || [],
    paragraph_time_indexes: post.paragraph_time_indexes || null,
    bias_label_percentages: post.bias_label_percentages || [],
    content_html: post.content_html || null,
    content_json: post.content_json || null,
    embedded_images: post.embedded_images || null,
